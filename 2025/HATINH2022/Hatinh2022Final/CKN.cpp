    ...
    for(int v:adj[u]){
        ...
        if(!num[v]){
            ...
            /* 
                ...
                Nghĩa là đỉnh v không thể đến được các cạnh cụ của u 
                Hoặc nói đơn giản là khi xóa u, khối mà v thuộc về tách
                ra làm một TPLT riêng.
                Trong trường hợp low[v]<num[u] nghĩa là v có thể đến được một 
                trong các cạnh cụ của u.
            */
        ...
        //Nếu cạnh v đã được thăm nhưng v không phải cha của u, nghĩa là v thuộc một cảnh vòng(nếu loại bỏ v thì các cạnh vòng dù có bao nhiêu điểm liền kề với u thì đều chỉ tính là 1 TPLT)
            ...
            low[u]=min(low[v],num[u]); //Quyết định lấy thời gian nhỏ nhất của v hoặc lấy thời điểm u được thăm đầu tiên
        }
    }
...
int main()
{
    ...
    //Duyệt qua mỗi đỉnh, đỉnh nào tarjan chưa đi qua thì bắt đầu tarjan với đỉnh đó là gốc
    for (int i = 1; i <= n; i++)
    {
        if(num[i]==0){
            tarjan(i,true);
